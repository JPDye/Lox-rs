...
type ParsingResult = Result<Expr, ParsingError>;
...
impl<'a> Parser<'a> {
    ...
    pub fn parse(&mut self) -> (Vec<Box<Expr>>, Vec<ParsingError>) {
        let mut expressions = Vec::new();
        let mut errors = Vec::new();

        while !self.finished() {
            match self.expression() {
                Ok(expr) => expressions.push(Box::new(expr)),
    ...
    /// term ---> factor ( ( "-" | "+" ) factor ) *
    fn term(&mut self) -> ParsingResult {
    ...
    /// factor ---> unary ( ( "/" | "*" ) unary ) *
    fn factor(&mut self) -> ParsingResult {
    ...
    fn primary(&mut self) -> ParsingResult {
        ...
        match self.peek().unwrap().kind {
            ...
            Kind::False | Kind::True | Kind::Nil | Kind::String(_) | Kind::Number(_) => {
                Ok(Expr::Literal { value: self.next() })
            }
...
mod tests {
    ...
    // Helper function. Create parse and parse input. Return result.
    fn parse_input(input: &str) -> (Vec<Box<Expr>>, Vec<ParsingError>) {
            ...
            expr: Box::new(Expr::Unary {
                ...
                expr: Box::new(Expr::Unary {
                    ...
                    expr: Box::new(Expr::Literal {
                        value: Token::new(Kind::Number(10.0), 3..5),
                    }),
                }),
            }),
        });
    ...
    fn parse_simple_expression() {
        ...
        let expected = Box::new(Expr::Binary {
            left: Box::new(Expr::Group {
                expr: Box::new(Expr::Binary {
                    left: Box::new(Expr::Literal {
                        value: Token::new(Kind::Number(10.0), 1..3),
                    }),
                    op: Token::new(Kind::Plus, 4..5),
                    right: Box::new(Expr::Literal {
                        value: Token::new(Kind::Number(5.0), 6..7),
                    }),
                }),
            }),
            ...
            right: Box::new(Expr::Group {
                expr: Box::new(Expr::Binary {
                    left: Box::new(Expr::Literal {
                        value: Token::new(Kind::Number(9.0), 12..13),
                    }),
                    op: Token::new(Kind::Minus, 14..15),
                    right: Box::new(Expr::Literal {
                        value: Token::new(Kind::Number(2.0), 16..17),
                    }),
                }),
            }),
        });
    ...
    fn parse_equality() {
        ...
        let expected = Box::new(Expr::Binary {
            left: Box::new(Expr::Literal {
                value: Token::new(Kind::Number(5.0), 0..1),
            }),

            op: Token::new(Kind::DoubleEq, 2..4),

            right: Box::new(Expr::Literal {
                value: Token::new(Kind::Number(10.0), 5..7),
            }),
        });
